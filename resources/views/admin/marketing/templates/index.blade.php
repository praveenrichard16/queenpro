			...
			<div class="d-flex justify-content-between align-items-center">
				<form method="GET" class="d-flex gap-3">
					<select class="form-select h-56-px bg-neutral-50 radius-12" name="type" onchange="this.form.submit()">
						...
						<option value="push_notification" @selected(request('type') === 'push_notification')>Push Notification</option>
					</select>
				</form>
				<div class="d-flex gap-2">
					@if(request('type') === 'whatsapp')
						<form action="{{ route('admin.marketing.templates.sync-whatsapp') }}" method="POST" class="d-inline">
							@csrf
					...
					@endif
					<a href="{{ route('admin.marketing.templates.create') }}" class="btn btn-warning radius-12 text-white h-56-px d-inline-flex align-items-center justify-content-center gap-2">
					...
					<tbody>
						@forelse($templates as $template)
							<tr>
								...
								<td>
									<span class="badge bg-{{ $template->type === 'email' ? 'primary' : ($template->type === 'whatsapp' ? 'success' : 'info') }}">
								...
								<td>
									@if($template->type === 'whatsapp')
										<div class="text-secondary-light small">
											<div>{{ strtoupper($template->language ?? 'EN') }}</div>
											<div>{{ ucfirst(strtolower($template->category ?? 'MARKETING')) }}</div>
										</div>
									@else
								...
								<td>
									@if($template->type === 'whatsapp' && $template->whatsapp_template_status)
										<span class="badge bg-{{ $template->whatsapp_template_status === 'approved' ? 'success' : ($template->whatsapp_template_status === 'rejected' ? 'danger' : 'warning') }}">
											{{ ucfirst($template->whatsapp_template_status) }}
										</span>
									@else
										<span class="text-secondary-light">—</span>
									@endif
								...
								<td class="text-end">
									<div class="d-inline-flex gap-2">
										<a href="{{ route('admin.marketing.templates.edit', $template) }}" class="w-36-px h-36-px bg-primary-focus text-primary-main rounded-circle d-inline-flex align-items-center justify-content-center">
											<iconify-icon icon="lucide:edit"></iconify-icon>
										</a>
										<form action="{{ route('admin.marketing.templates.destroy', $template) }}" method="POST" onsubmit="return confirm('Delete this template?')" class="d-inline">
											@csrf
											@method('DELETE')
