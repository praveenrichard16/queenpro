	...
	<div class="row">
			...
			<div class="card border-0 h-100">
				<div class="card-body p-16">
					<div class="nav flex-column nav-pills gap-2" id="home-tabs" role="tablist" aria-orientation="vertical">
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab', 'hero') === 'hero' ? 'active' : '' }}" id="home-tab-hero" data-bs-toggle="pill" data-bs-target="#home-pane-hero" type="button" role="tab" aria-controls="home-pane-hero" aria-selected="{{ session('active_tab', 'hero') === 'hero' ? 'true' : 'false' }}">
							<span>Home Hero Slider</span>
							<iconify-icon icon="solar:slider-vertical-linear" class="text-lg"></iconify-icon>
						</button>
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab') === 'second' ? 'active' : '' }}" id="home-tab-second" data-bs-toggle="pill" data-bs-target="#home-pane-second" type="button" role="tab" aria-controls="home-pane-second" aria-selected="{{ session('active_tab') === 'second' ? 'true' : 'false' }}">
							<span>Second Section Slider</span>
							<iconify-icon icon="solar:image-bold-duotone" class="text-lg"></iconify-icon>
						</button>
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab') === 'product' ? 'active' : '' }}" id="home-tab-product" data-bs-toggle="pill" data-bs-target="#home-pane-product" type="button" role="tab" aria-controls="home-pane-product" aria-selected="{{ session('active_tab') === 'product' ? 'true' : 'false' }}">
							<span>Product Slider</span>
							<iconify-icon icon="solar:bag-heart-linear" class="text-lg"></iconify-icon>
						</button>
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab') === 'third' ? 'active' : '' }}" id="home-tab-third" data-bs-toggle="pill" data-bs-target="#home-pane-third" type="button" role="tab" aria-controls="home-pane-third" aria-selected="{{ session('active_tab') === 'third' ? 'true' : 'false' }}">
							<span>3rd Section Slider</span>
							<iconify-icon icon="solar:image-bold-duotone" class="text-lg"></iconify-icon>
						</button>
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab') === 'product2' ? 'active' : '' }}" id="home-tab-product2" data-bs-toggle="pill" data-bs-target="#home-pane-product2" type="button" role="tab" aria-controls="home-pane-product2" aria-selected="{{ session('active_tab') === 'product2' ? 'true' : 'false' }}">
							<span>Product Slider 2</span>
							<iconify-icon icon="solar:bag-4-linear" class="text-lg"></iconify-icon>
						</button>
						<button class="btn btn-outline-secondary d-flex justify-content-between align-items-center {{ session('active_tab') === 'reviews' ? 'active' : '' }}" id="home-tab-reviews" data-bs-toggle="pill" data-bs-target="#home-pane-reviews" type="button" role="tab" aria-controls="home-pane-reviews" aria-selected="{{ session('active_tab') === 'reviews' ? 'true' : 'false' }}">
							<span>Reviews</span>
		...
		<div class="col-lg-9">
			<div class="tab-content" id="home-tabs-content">
				...
				<div class="tab-pane fade {{ session('active_tab', 'hero') === 'hero' ? 'show active' : '' }}" id="home-pane-hero" role="tabpanel" aria-labelledby="home-tab-hero">
						...
						<div class="card-body p-24">
							<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-16">
								...
								<div class="d-flex gap-2">
									<a href="{{ route('admin.cms.home.seo.settings') }}" class="btn btn-outline-secondary radius-12">
										<iconify-icon icon="solar:seo-linear" class="me-1"></iconify-icon>
										SEO Settings
									</a>
									<a href="{{ route('admin.cms.home.sliders.create') }}?active_tab=hero"
									   class="btn btn-primary radius-12 {{ $canCreate ? '' : 'disabled' }}"
										...
										<tr>
											...
											<td class="text-end">
												<div class="d-inline-flex gap-2">
													<a href="{{ route('admin.cms.home.sliders.edit', $slider) }}?active_tab=hero" class="btn btn-sm btn-outline-secondary">
														Edit
													</a>
													<form action="{{ route('admin.cms.home.sliders.destroy', $slider) }}" method="POST" onsubmit="return confirm('Delete this slide?');" class="d-inline">
														@csrf
														@method('DELETE')
				...
				<div class="tab-pane fade {{ session('active_tab') === 'second' ? 'show active' : '' }}" id="home-pane-second" role="tabpanel" aria-labelledby="home-tab-second">
					<div class="card border-0 mb-24">
						<div class="card-body p-24">
							<div class="mb-16">
				...
				<!-- Product Slider Tab -->
				<div class="tab-pane fade {{ session('active_tab') === 'product' ? 'show active' : '' }}" id="home-pane-product" role="tabpanel" aria-labelledby="home-tab-product">
					<div class="card border-0 mb-24">
						<div class="card-body p-24">
							<div class="mb-16">
				...
				<div class="tab-pane fade {{ session('active_tab') === 'third' ? 'show active' : '' }}" id="home-pane-third" role="tabpanel" aria-labelledby="home-tab-third">
					<div class="card border-0 mb-24">
						<div class="card-body p-24">
							<div class="mb-16">
							...
							<form method="POST"
								  action="{{ route('admin.cms.home.third-slider.update') }}"
								  enctype="multipart/form-data"
								  class="row g-4">
								@csrf
								...
								<div class="col-12">
									<div class="form-check form-switch">
										<input type="checkbox"
											   ...
											   value="1"
											   {{ old('is_active', $thirdSlider['is_active']) ? 'checked' : '' }}>
								...
								<div class="col-md-6">
									...
									@error('desktop_image') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
									<div class="form-text mt-1">Upload JPG/PNG/WebP up to 4MB. This image is shown on laptop & desktop screens.</div>
									@if($thirdSlider['desktop_image_path'])
										<div class="mt-3">
											<div class="text-secondary-light mb-2">Current desktop preview</div>
								...
								<div class="col-md-6">
									...
									@error('mobile_image') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
									<div class="form-text mt-1">Upload JPG/PNG/WebP up to 4MB that fits portrait screens.</div>
									@if($thirdSlider['mobile_image_path'])
										<div class="mt-3">
											<div class="text-secondary-light mb-2">Current mobile preview</div>
											<img src="{{ asset('storage/' . $thirdSlider['mobile_image_path']) }}"
												 alt="Current mobile visual"
												 class="img-fluid rounded-3 border"
												 style="max-height:240px;object-fit:cover;">
										</div>
								...
								<div class="col-md-6">
									...
									<input type="text"
										   name="alt_text"
										   class="form-control bg-neutral-50 radius-12 h-56-px @error('alt_text') is-invalid @enderror"
										   value="{{ old('alt_text', $thirdSlider['alt_text']) }}"
										   placeholder="Describe the visual for accessibility">
									@error('alt_text') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
								</div>
								...
								<div class="col-md-6">
									...
									<input type="url"
										   name="button_link"
										   class="form-control bg-neutral-50 radius-12 h-56-px @error('button_link') is-invalid @enderror"
										   value="{{ old('button_link', $thirdSlider['button_link']) }}"
										   placeholder="https://example.com/collection">
									@error('button_link') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
									<div class="form-text mt-1">Provide a URL to make the slider clickable.</div>
								</div>
				...
				<div class="tab-pane fade {{ session('active_tab') === 'product2' ? 'show active' : '' }}" id="home-pane-product2" role="tabpanel" aria-labelledby="home-tab-product2">
					<div class="card border-0 mb-24">
						<div class="card-body p-24">
							<div class="mb-16">
							...
							<form method="POST" action="{{ route('admin.cms.home.product-slider-2.update') }}" class="row g-4">
								@csrf
								...
								<div class="col-lg-5">
									...
									<div class="form-text mt-2" id="productSlider2LimitMessage">
										You can feature up to {{ $maxProductSlider2 }} products. Order is preserved.
									</div>
									@error('product_slider2') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
									@error('product_slider2.*') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
								</div>
								...
								<div class="col-lg-7">
									...
									<div id="selectedProductSlider2Empty" class="border rounded-3 p-4 text-center text-secondary-light {{ $productSlider2Selected->isNotEmpty() ? 'd-none' : '' }}" style="border-style: dashed;">
										No products selected yet.
									</div>

									<ul class="list-group gap-3" id="selectedProductSlider2List">
										@foreach($productSlider2Selected as $product)
											<li class="list-group-item d-flex align-items-center justify-content-between radius-12 border flex-wrap gap-3 product-slider2-item" data-id="{{ $product->id }}">
												<div class="d-flex align-items-center gap-3">
													<span class="badge bg-neutral-100 text-secondary-light border order-index"></span>
													<div>
														<div class="fw-semibold">{{ $product->name }}</div>
				...
				<!-- Reviews Tab -->
				<div class="tab-pane fade {{ session('active_tab') === 'reviews' ? 'show active' : '' }}" id="home-pane-reviews" role="tabpanel" aria-labelledby="home-tab-reviews">
					<div class="card border-0 mb-24">
						<div class="card-body p-24">
							<div class="mb-16">
							...
							<form method="POST" action="{{ route('admin.cms.home.reviews.update') }}" class="row g-4">
								@csrf
								...
								<div class="col-12">
									...
									<input type="text"
										   name="title"
										   class="form-control bg-neutral-50 radius-12 h-56-px @error('title') is-invalid @enderror"
										   value="{{ old('title', $homeReviewsTitle) }}"
								...
								<div class="col-12">
									...
									<textarea name="description"
											  rows="3"
											  class="form-control bg-neutral-50 radius-12 @error('description') is-invalid @enderror"
											  placeholder="Read what our satisfied customers have to say about their shopping experience.">{{ old('description', $homeReviewsDescription) }}</textarea>
								...
								<div class="col-lg-5">
									...
									<select id="reviewSelect" class="form-select bg-neutral-50 radius-12 h-56-px">
										...
										@foreach($reviewOptions as $review)
											<option value="{{ $review->id }}"
													...
													data-comment="{{ Str::limit($review->comment ?? '', 50) }}">
												{{ $review->customer_name }} - {{ $review->product->name ?? 'N/A' }} ({{ $review->rating }}★)
											</option>
										@endforeach
									</select>
									<div class="d-flex gap-2 mt-3">
									...
									<div class="form-text mt-2" id="reviewsLimitMessage">
										You can feature up to {{ $maxHomeReviews }} reviews. Order is preserved.
									</div>
									@error('home_reviews') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
									@error('home_reviews.*') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
								</div>
								...
								<div class="col-lg-7">
									...
									<div id="selectedReviewsEmpty" class="border rounded-3 p-4 text-center text-secondary-light {{ $homeReviewsSelected->isNotEmpty() ? 'd-none' : '' }}" style="border-style: dashed;">
										No reviews selected yet.
									</div>

									<ul class="list-group gap-3" id="selectedReviewsList">
										@foreach($homeReviewsSelected as $review)
											<li class="list-group-item d-flex align-items-center justify-content-between radius-12 border flex-wrap gap-3 review-item" data-id="{{ $review->id }}">
												<div class="d-flex align-items-center gap-3 flex-grow-1">
													...
													<div class="flex-grow-1">
														...
														<div class="text-secondary-light small">
															@for($i = 1; $i <= 5; $i++)
																<iconify-icon icon="solar:star-bold" class="{{ $i <= $review->rating ? 'text-warning' : 'text-secondary' }}"></iconify-icon>
															@endfor
															@if($review->product)
																<span class="ms-2">- {{ $review->product->name }}</span>
															@endif
														</div>
														@if($review->title)
															<div class="text-secondary-light small mt-1">{{ Str::limit($review->title, 60) }}</div>
														@endif
													</div>
												</div>
...
<script>
	...
	const maxProductSlider2 = {{ $maxProductSlider2 }};
	...
	const selectedProductSlider2Ids = new Set(@json($productSlider2Selected->pluck('id')));
	...
	const maxHomeReviews = {{ $maxHomeReviews }};
	...
	const selectedReviewIds = new Set(@json($homeReviewsSelected->pluck('id')));
