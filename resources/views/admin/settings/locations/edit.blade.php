...
<div class="dashboard-main-body">
	<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
		<div>
			...
			<p class="text-secondary-light mb-0">Update location information.</p>
		</div>
	...
	<div class="card border-0 shadow-sm">
		<div class="card-body p-24">
			<form action="{{ route('admin.settings.locations.update', $location) }}" method="POST" class="row g-4">
				@csrf
				@method('PUT')
				<div class="col-md-6">
					...
					<input type="text" 
						   ...
						   autocomplete="off"
						   value="{{ old('region_name_en', $location->region_name_en) }}" 
						   required>
				...
				<div class="col-md-6">
					...
					<input type="text" 
						   ...
						   autocomplete="off"
						   value="{{ old('city_name_en', $location->city_name_en) }}" 
						   required>
				...
				<div class="col-md-6">
					...
					<input type="text" name="district_name_en" class="form-control bg-neutral-50 radius-12 @error('district_name_en') is-invalid @enderror" value="{{ old('district_name_en', $location->district_name_en) }}">
				...
				<div class="col-md-4">
					...
					<input type="number" step="any" name="latitude" class="form-control bg-neutral-50 radius-12 @error('latitude') is-invalid @enderror" value="{{ old('latitude', $location->latitude) }}" min="-90" max="90">
					@error('latitude') <div class="invalid-feedback">{{ $message }}</div> @enderror
				</div>
				<div class="col-md-4">
					<label class="form-label text-secondary-light">Longitude</label>
					<input type="number" step="any" name="longitude" class="form-control bg-neutral-50 radius-12 @error('longitude') is-invalid @enderror" value="{{ old('longitude', $location->longitude) }}" min="-180" max="180">
					@error('longitude') <div class="invalid-feedback">{{ $message }}</div> @enderror
				</div>
				<div class="col-12">
					<div class="form-check form-switch">
						<input class="form-check-input" type="checkbox" id="location-active" name="is_active" value="1" {{ old('is_active', $location->is_active) ? 'checked' : '' }}>
						<label class="form-check-label text-secondary-light" for="location-active">Location is active</label>
					</div>
				</div>
				<div class="col-12">
					<button class="btn btn-primary radius-12 px-24" type="submit">Update Location</button>
				</div>
			</form>
		</div>
	</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('{{ route("api.india.states") }}')
        ...
        .then(data => {
            ...
            // Load cities for current region if exists
            const currentRegion = document.getElementById('region_name_en').value;
            if (currentRegion && statesData.includes(currentRegion)) {
                loadCities(currentRegion);
            }
        })
    ...
    function loadPostalCode(city, region) {
        fetch(`{{ route("api.india.postal-codes") }}?city=${encodeURIComponent(city)}&state=${encodeURIComponent(region)}`)
            .then(response => response.json())
            .then(data => {
                if (data.length > 0 && !document.getElementById('postal_code').value) {
