		...
		<div class="card-body p-24">
			<form method="GET" action="{{ route('admin.users.index') }}" class="row gy-3 gx-3 align-items-end">
				...
				<div class="col-xl-3 col-lg-4 d-flex gap-2">
					...
					<a href="{{ route('admin.users.index', ['tab' => 'admins']) }}" class="btn btn-outline-secondary h-56-px radius-12">
						<iconify-icon icon="mi:refresh"></iconify-icon>
					</a>
				</div>
				<div class="col-xl-3 col-lg-12 text-lg-end">
					<a href="{{ route('admin.users.create', ['role' => 'admin']) }}" class="btn btn-warning radius-12 text-white d-inline-flex align-items-center gap-2 h-56-px">
					...
					<tbody>
						@forelse($users ?? [] as $user)
							<tr>
								...
								<td>
									<span class="px-20 py-6 rounded-pill fw-semibold text-sm {{ $user->is_super_admin ? 'bg-danger-focus text-danger-main' : 'bg-primary-focus text-primary-main' }}">
								...
								<td class="text-end">
									<div class="d-inline-flex gap-2">
										<a href="{{ route('admin.users.admin-activity', $user) }}" class="w-36-px h-36-px bg-info-focus text-info-main rounded-circle d-inline-flex align-items-center justify-content-center" title="View Activity">
										...
										@if(!$user->is_super_admin)
											<form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Delete this admin?')" class="d-inline">
												@csrf
												@method('DELETE')
						...
						@empty
							<tr>
								<td colspan="6" class="text-center py-80">
									<img src="{{ asset('wowdash/assets/images/notification/empty-message-icon1.png') }}" alt="No admins" class="mb-16" style="max-width:120px;">
